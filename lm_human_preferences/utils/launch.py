...
def launch(name, f, *, namespace='safety', mode='local', mpi=1) -> None:
    if mode == 'local':
        with open('/home/badri/mansion/openai-training/lm-human-preferences/tmp/pickle_fn', 'wb') as file:
            cloudpickle.dump(f, file)

        subprocess.check_call(['mpiexec', '-n', str(mpi), 'python', '-c', 'import sys; import pickle; pickle.loads(open("/home/badri/mansion/openai-training/lm-human-preferences/tmp/pickle_fn", "rb").read())()'])
        return
